...
// === Inicia ou retoma uma resposta do Big Five Inventory (BFI) ===
//
// Espelha o smi-assessment-start (mesma lógica de match/criação de cliente
// e o mesmo suporte a invite_token do link individual gerado pelo
// terapeuta), mas grava em client_bfi_assessments — trilha própria, já que
// o BFI numera perguntas 1..25 e reaproveitar outra tabela colidiria.
//
...
// início do BFI). Fire-and-forget: uma falha aqui não deve impedir o
// cliente de continuar o formulário.
async function notifyNewSignup(name: string, email: string, whatsapp: string) {
  if (!ZEPTOMAIL_API_KEY) {
    console.warn("[bfi-assessment-start] ZEPTOMAIL_API_KEY não configurada, pulando notificação");
    return;
  }
  try {
      ...
      body: JSON.stringify({
        ...
        htmlbody: `
          <div style="font-family: sans-serif; color: #2C2C2C; line-height: 1.6;">
            <p>Um novo cliente se cadastrou pelo Big Five Inventory (BFI):</p>
            <ul>
    ...
    if (!res.ok) {
      console.error("[bfi-assessment-start] ZeptoMail respondeu com erro na notificação:", res.status, await res.text());
    }
  } catch (err) {
    console.error("[bfi-assessment-start] Falha ao notificar novo cadastro:", err);
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (tokenError || !editToken) {
      await supabase.from("client_bfi_assessments").delete().eq("id", assessment.id);
      throw tokenError ?? new Error("Falha ao emitir token de edição");
    }

    console.log(`[bfi-assessment-start] assessment ${assessment.id} (v${version}) iniciado para ${email}`);

    return json({ client_id: clientId, assessment_id: assessment.id, edit_token: editToken, version, raw_answers: {} });
  } catch (err) {
    console.error("[bfi-assessment-start] Erro inesperado:", err);
