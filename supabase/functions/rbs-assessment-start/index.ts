...
// === Inicia ou retoma uma resposta da Escala de Crenças Românticas (RBS) ===
//
...
// terapeuta), mas grava em client_rbs_assessments — trilha própria.
//
...
async function notifyNewSignup(name: string, email: string, whatsapp: string) {
  if (!ZEPTOMAIL_API_KEY) {
    console.warn("[rbs-assessment-start] ZEPTOMAIL_API_KEY não configurada, pulando notificação");
    return;
  }
  try {
      ...
      body: JSON.stringify({
        ...
        htmlbody: `
          <div style="font-family: sans-serif; color: #2C2C2C; line-height: 1.6;">
            <p>Um novo cliente se cadastrou pela Escala de Crenças Românticas (RBS):</p>
            <ul>
    ...
    if (!res.ok) {
      console.error("[rbs-assessment-start] ZeptoMail respondeu com erro na notificação:", res.status, await res.text());
    }
  } catch (err) {
    console.error("[rbs-assessment-start] Falha ao notificar novo cadastro:", err);
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (tokenError || !editToken) {
      await supabase.from("client_rbs_assessments").delete().eq("id", assessment.id);
      throw tokenError ?? new Error("Falha ao emitir token de edição");
    }

    console.log(`[rbs-assessment-start] assessment ${assessment.id} (v${version}) iniciado para ${email}`);

    return json({ client_id: clientId, assessment_id: assessment.id, edit_token: editToken, version, raw_answers: {} });
  } catch (err) {
    console.error("[rbs-assessment-start] Erro inesperado:", err);
