...
// === Inicia ou retoma uma resposta da ETAS-R (amor triangular) ===
//
...
// terapeuta), mas grava em client_etas_assessments — trilha própria.
//
...
async function notifyNewSignup(name: string, email: string, whatsapp: string) {
  if (!ZEPTOMAIL_API_KEY) {
    console.warn("[etas-assessment-start] ZEPTOMAIL_API_KEY não configurada, pulando notificação");
    return;
  }
  try {
      ...
      body: JSON.stringify({
        ...
        htmlbody: `
          <div style="font-family: sans-serif; color: #2C2C2C; line-height: 1.6;">
            <p>Um novo cliente se cadastrou pela ETAS-R:</p>
            <ul>
    ...
    if (!res.ok) {
      console.error("[etas-assessment-start] ZeptoMail respondeu com erro na notificação:", res.status, await res.text());
    }
  } catch (err) {
    console.error("[etas-assessment-start] Falha ao notificar novo cadastro:", err);
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (tokenError || !editToken) {
      await supabase.from("client_etas_assessments").delete().eq("id", assessment.id);
      throw tokenError ?? new Error("Falha ao emitir token de edição");
    }

    console.log(`[etas-assessment-start] assessment ${assessment.id} (v${version}) iniciado para ${email}`);

    return json({ client_id: clientId, assessment_id: assessment.id, edit_token: editToken, version, raw_answers: {} });
  } catch (err) {
    console.error("[etas-assessment-start] Erro inesperado:", err);
