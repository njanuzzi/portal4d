...
// === Inicia ou retoma uma resposta da ECR-R (apego adulto) ===
//
...
// terapeuta), mas grava em client_ecr_assessments — trilha própria.
//
...
async function notifyNewSignup(name: string, email: string, whatsapp: string) {
  if (!ZEPTOMAIL_API_KEY) {
    console.warn("[ecr-assessment-start] ZEPTOMAIL_API_KEY não configurada, pulando notificação");
    return;
  }
  try {
      ...
      body: JSON.stringify({
        ...
        htmlbody: `
          <div style="font-family: sans-serif; color: #2C2C2C; line-height: 1.6;">
            <p>Um novo cliente se cadastrou pela ECR-R:</p>
            <ul>
    ...
    if (!res.ok) {
      console.error("[ecr-assessment-start] ZeptoMail respondeu com erro na notificação:", res.status, await res.text());
    }
  } catch (err) {
    console.error("[ecr-assessment-start] Falha ao notificar novo cadastro:", err);
  }
}
