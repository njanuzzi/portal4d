...
const STORAGE_KEY = 'portal_ecr_draft';
...
export function ECRQuestionnaire() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (draft?.assessment_id && draft?.edit_token) {
        const { data, error: fnError } = await supabase.functions.invoke('ecr-assessment-start', {
  ...
  const handleSubmit = async () => {
    ...
    const { data, error: fnError } = await supabase.functions.invoke('ecr-assessment-save', {
  ...
  return (
    <div className="min-h-screen bg-[#032a3c] text-white font-sans">
      ...
      <div className="max-w-md mx-auto px-6 py-10">
        ...
        {step === 'identity' && (
          <>
            <h1 className="font-serif text-2xl text-center mb-3 text-balance">Experiências em Relacionamentos Íntimos</h1>
            <p className="text-petrol-100/70 text-sm text-center leading-relaxed mb-8">
              Esse questionário tem 10 perguntas e leva de 3 a 5 minutos.
            </p>
        ...
        {step === 'welcome' && (
          <>
            ...
            <div className="space-y-5 text-sm text-petrol-100/85 leading-relaxed mb-8">
              <p>
                Por favor, leia as afirmações a seguir e marque o quanto cada uma descreve as emoções e sentimentos
                que você geralmente tem em relacionamentos amorosos e/ou sexuais. Queremos saber como você se sente
                em relacionamentos de modo geral, não apenas no seu relacionamento atual. Mesmo que você nunca
                tenha tido um relacionamento, responda imaginando como você se sentiria se estivesse em um.
              </p>
            </div>
          ...
          <>
            <div className="mb-6">
              <div className="flex items-center justify-between text-xs text-petrol-100/60 mb-1.5">
                <span>Experiências em Relacionamentos Íntimos</span>
