...
export function MARQResponseDetail() {
    ...
    const load = async () => {
      ...
      const [{ data: assessmentRow }, { data: questionRows }] = await Promise.all([
        supabase
          ...
          .eq('id', assessmentId)
          .maybeSingle(),
        supabase.from('marq_questions').select('question_number, question_text').order('question_number'),
      ]);
  ...
  const handleDownload = () => {
    ...
    downloadCsv(`respostas-marq-${assessment.client_name.replace(/\s+/g, '-').toLowerCase()}-v${assessment.version}.csv`, csv);
  };
  ...
  if (!assessment) {
    return (
      <div className="p-6 max-w-3xl mx-auto">
        <Link to="/marq-respostas" className="flex items-center gap-2 text-sm text-dark/50 hover:text-petrol-700 transition-colors mb-4">
          <ArrowLeft size={16} />
          Voltar
        </Link>
  ...
  const scores = computeMarqScores(assessment.raw_answers);
    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-start gap-2.5 bg-beige-50 border border-beige-200 rounded-lg px-4 py-3 mb-6">
        ...
        <p className="text-xs text-dark/60 leading-relaxed">
          O MARQ não tem pontos de corte validados para a população brasileira — use os escores abaixo como
          hipótese complementar à entrevista clínica do casal, não como medida isolada.
        </p>
      </div>
      ...
      {incomplete ? (
        ...
        <div className="grid grid-cols-2 gap-4">
          <Card>
            <CardBody>
              <p className="text-xs font-medium text-dark/40 mb-1">Escala global</p>
              <p className="text-2xl font-semibold text-dark font-serif">{scores.global}</p>
              <p className="text-xs text-dark/40 mt-1">Soma das 9 questões (9 a 45)</p>
            </CardBody>
          </Card>
          <Card>
            <CardBody>
              <p className="text-xs font-medium text-dark/40 mb-1">Escore total</p>
              <p className="text-2xl font-semibold text-dark font-serif">{scores.escoreTotal.toFixed(2)}/5</p>
              <p className="text-xs text-dark/40 mt-1">Média das 9 questões</p>
            </CardBody>
          </Card>
        </div>
      )}
