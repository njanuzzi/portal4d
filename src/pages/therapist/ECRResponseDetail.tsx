...
export function ECRResponseDetail() {
    ...
    const load = async () => {
      ...
      const [{ data: assessmentRow }, { data: questionRows }] = await Promise.all([
        supabase
          ...
          .eq('id', assessmentId)
          .maybeSingle(),
        supabase.from('ecr_questions').select('question_number, question_text').order('question_number'),
      ]);
  ...
  const handleDownload = () => {
    ...
    downloadCsv(`respostas-ecr-${assessment.client_name.replace(/\s+/g, '-').toLowerCase()}-v${assessment.version}.csv`, csv);
  };
  ...
  if (!assessment) {
    return (
      <div className="p-6 max-w-3xl mx-auto">
        <Link to="/ecr-respostas" className="flex items-center gap-2 text-sm text-dark/50 hover:text-petrol-700 transition-colors mb-4">
          <ArrowLeft size={16} />
          Voltar
        </Link>
  ...
  const subscales = computeEcrScores(assessment.raw_answers);
    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-start gap-2.5 bg-beige-50 border border-beige-200 rounded-lg px-4 py-3 mb-6">
        ...
        <p className="text-xs text-dark/60 leading-relaxed">
          A ECR-R não tem pontos de corte validados para a população brasileira — as duas dimensões (Ansiedade e
          Evitação) são relativamente independentes e devem ser lidas separadamente, não como medida isolada.
        </p>
      </div>
      ...
      {incomplete ? (
        ...
        <div className="grid grid-cols-2 gap-4">
          {subscales.map((s) => (
            <Card key={s.key}>
              <CardBody>
                <p className="text-xs font-medium text-dark/40 mb-1">{s.label}</p>
                <p className="text-2xl font-semibold text-dark font-serif">{s.sum}</p>
                <p className="text-xs text-dark/40 mt-1">Soma de 5 itens (5 a 35)</p>
              </CardBody>
            </Card>
          ))}
        </div>
