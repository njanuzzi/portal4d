...
function extractReportContent(content: string) {
  const container = document.createElement('div');
  container.innerHTML = content;
  const heading = container.querySelector('h2');
  const title = heading?.textContent?.trim() || stripHtml(content).split('\n')[0] || 'Relatório clínico';
  heading?.remove();
  return {
    title,
...
export function EditReport() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [{ data: clientRow, error: clientError }, { data: reportRow, error: reportError }] = await Promise.all([
        supabase
          .from('profiles')
          .select('*')
          .eq('id', clientId)
      ...
      const loadedReport = reportRow as ReportRow | null;
      const extracted = loadedReport ? extractReportContent(loadedReport.content_text) : { title: '', bodyHtml: '' };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (updateError) {
      setError(updateError.message ?? 'Não foi possível atualizar o relatório.');
      setLoading(false);
      return;
    }
  ...
  if (!client || !report) {
    return (
      <div className="p-6 max-w-2xl mx-auto">
        <Link to={clientId ? `/reports/${clientId}` : '/reports'} className="flex items-center gap-2 text-sm text-dark/50 hover:text-petrol-700 transition-colors mb-4">
          <ArrowLeft size={16} />
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-semibold text-dark font-serif">Editar Relatório</h1>
