...
import { ArrowLeft, Smile, Download, Info } from 'lucide-react';
import { Card, CardBody } from '../../components/ui/Card';
import { Button } from '../../components/ui/Button';
import { PageSpinner } from '../../components/ui/Spinner';
import { supabase } from '../../lib/supabase';
import { buildRawAnswersCsv, downloadCsv, QuestionRef } from '../../lib/schemaCsv';
import { computeEnsraScore } from '../../lib/ensraScore';
...
export function ENSRAResponseDetail() {
    ...
    const load = async () => {
      ...
      const [{ data: assessmentRow }, { data: questionRows }] = await Promise.all([
        supabase
          ...
          .eq('id', assessmentId)
          .maybeSingle(),
        supabase.from('ensra_questions').select('question_number, question_text').order('question_number'),
      ]);
  ...
  const handleDownload = () => {
    ...
    downloadCsv(`respostas-ensra-${assessment.client_name.replace(/\s+/g, '-').toLowerCase()}-v${assessment.version}.csv`, csv);
  };
  ...
  if (!assessment) {
    return (
      <div className="p-6 max-w-3xl mx-auto">
        <Link to="/ensra-respostas" className="flex items-center gap-2 text-sm text-dark/50 hover:text-petrol-700 transition-colors mb-4">
          <ArrowLeft size={16} />
          Voltar
        </Link>
  ...
  const global = computeEnsraScore(assessment.raw_answers);
    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-start gap-2.5 bg-beige-50 border border-beige-200 rounded-lg px-4 py-3 mb-6">
        ...
        <p className="text-xs text-dark/60 leading-relaxed">
          A ENSRA-R não tem pontos de corte validados para a população brasileira — use como indicador global de
          satisfação, não como medida isolada.
        </p>
      </div>
      ...
      {incomplete ? (
        ...
        <Card>
          <CardBody>
            ...
            <p className="text-2xl font-semibold text-dark font-serif">{global.toFixed(2)}/8</p>
            <p className="text-xs text-dark/40 mt-1">Média dos 5 itens</p>
          </CardBody>
        </Card>
      )}
    </div>
