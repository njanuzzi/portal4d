...
import { AlertCircle, BookOpen, CheckCircle, KeyRound } from 'lucide-react';
...
export function ClientDiaryForm() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (diaryData.today_entry_id) {
        // Already submitted today — populate answers for the summary view
        ...
        setAnswers(
          diaryData.questions.map(q => ({ question_id: q.id, answer_text: '', answer_value: null }))
        );
      }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (rpcError || !result || result.error) {
      // "entry_exists" means they submitted concurrently — treat as success
