...
const STORAGE_KEY = 'portal_ensra_draft';
const SCALE_VALUES = [0, 1, 2, 3, 4, 5, 6, 7, 8];
...
export function ENSRAQuestionnaire() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (draft?.assessment_id && draft?.edit_token) {
        const { data, error: fnError } = await supabase.functions.invoke('ensra-assessment-start', {
  ...
  const handleSubmit = async () => {
    ...
    const { data, error: fnError } = await supabase.functions.invoke('ensra-assessment-save', {
  ...
  return (
    <div className="min-h-screen bg-[#032a3c] text-white font-sans">
      ...
      <div className="max-w-md mx-auto px-6 py-10">
        ...
        {step === 'identity' && (
          <>
            <h1 className="font-serif text-2xl text-center mb-3 text-balance">Satisfação com o Relacionamento</h1>
            <p className="text-petrol-100/70 text-sm text-center leading-relaxed mb-8">
              Esse questionário tem 5 perguntas e leva cerca de 3 minutos.
            </p>
        ...
        {step === 'welcome' && (
          <>
            ...
            <div className="space-y-5 text-sm text-petrol-100/85 leading-relaxed mb-8">
              <p>
                A seguir, você encontrará uma série de afirmações sobre relacionamentos amorosos. Por favor, leia
                cada uma delas e responda de acordo com a sua opinião. Pensando no(a) seu(sua) parceiro(a), indique
                o quanto você concorda com cada uma das afirmações.
              </p>
            </div>
          ...
          <>
            <div className="mb-6">
              <div className="flex items-center justify-between text-xs text-petrol-100/60 mb-1.5">
                <span>Satisfação com o Relacionamento</span>
            ...
            <div className="space-y-7 mb-6">
              {questions.map((q, idx) => (
                <div key={q.id}>
                  <p className="text-sm text-petrol-50 leading-relaxed mb-3">
                    {idx + 1}. {q.question_text}
                  </p>
                  <div className="flex items-center justify-between gap-1">
                    {SCALE_VALUES.map((value) => {
                      const selected = answers[q.question_number] === value;
                      return (
                        <button
                          key={value}
                          type="button"
                          onClick={() => setAnswer(q.question_number, value)}
                          className={`w-8 h-8 rounded-full text-xs font-semibold transition-all ${
                            selected
                              ? 'bg-gold-400 text-petrol-900 shadow-md scale-110'
                              : 'bg-petrol-800 text-petrol-200 border-2 border-petrol-600 hover:border-petrol-400'
                          }`}
                        >
                          {value}
                        </button>
                  ...
                  <div className="flex items-center justify-between text-[11px] text-petrol-100/50 mt-1.5 px-1">
