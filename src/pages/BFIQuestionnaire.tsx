...
const STORAGE_KEY = 'portal_bfi_draft';

// Escala de 5 pontos do BFI — de concordância, diferente das escalas de
// frequência/veracidade do SMI/YSQ.
const SCALE = [
  { value: 1, label: 'Discordo totalmente' },
  { value: 2, label: 'Discordo um pouco' },
  { value: 3, label: 'Nem concordo, nem discordo' },
  { value: 4, label: 'Concordo um pouco' },
...
export function BFIQuestionnaire() {
  ...
  // Carrega as 25 perguntas (público, sem login) e tenta retomar um rascunho
  // salvo no localStorage — igual o acesso sem login do SMI/YSQ.
  useEffect(() => {
    const load = async () => {
      ...
      if (draft?.assessment_id) {
        const { data, error: fnError } = await supabase.functions.invoke('bfi-assessment-start', {
  ...
  const handleSubmit = async () => {
    ...
    const { data, error: fnError } = await supabase.functions.invoke('bfi-assessment-save', {
  ...
  return (
    <div className="min-h-screen bg-[#032a3c] text-white font-sans">
      ...
      <div className="max-w-md mx-auto px-6 py-10">
        ...
        {step === 'identity' && (
          <>
            <h1 className="font-serif text-2xl text-center mb-3 text-balance">Big Five Inventory</h1>
            <p className="text-petrol-100/70 text-sm text-center leading-relaxed mb-8">
              Esse questionário tem 25 perguntas e leva de 10 a 12 minutos. Você pode fechar e voltar depois —
              suas respostas ficam salvas automaticamente.
            </p>
        ...
        {step === 'welcome' && (
          <>
            ...
            <div className="space-y-5 text-sm text-petrol-100/85 leading-relaxed mb-8">
              <p>
                A seguir você verá características que podem ou não se aplicar a você. Por exemplo: você concorda
                que é alguém que gosta de passar tempo com outras pessoas?
              </p>
              <p>
                Selecione a opção que melhor descreve o quanto você concorda ou discorda de cada afirmação. Não
                existem respostas certas ou erradas — responda com base em como você é a maior parte do tempo.
              </p>
            </div>
        ...
        {step === 'quiz' && (
          <>
            ...
            <p className="text-sm text-petrol-100/70 leading-relaxed mb-6">
